...
skin. See {doc}`/reference/execution_support_matrix` for the public support
boundary and {doc}`/streamed_edge_execution` for architecture and diagnostics.
...
version, and the output of `backend list`. The `non-compiled` mode is a
diagnostic and compatibility route, not a direct-performance substitute.
